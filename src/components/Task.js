...
export default class Task {
    ...
    displayTask() {
        ...
        const borderColor = categoryColors[this.category];
        taskContainer.className = `flex border-2 border-[${borderColor}]`;

        const leftDiv = document.createElement("div");
        leftDiv.className = "flex flex-col gap-2";

        const title = document.createElement("p");
        title.textContent = util.capitalize(this.name);
        title.className = "font-medium text-xl text-textPrimary";
        leftDiv.appendChild(title);

        const description = document.createElement("p");
        description.textContent = util.capitalize(this.description);
        description.className =
            "text-sm text-slate-600 whitespace-nowrap text-ellipsis";
        ...
        if (this.endDate) {
            ...
            dateIcon.innerHTML = `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M3 5.5L5 3.5M21 5.5L19 3.5M9 12.5L11 14.5L15 10.5M20 12.5C20 16.9183 16.4183 20.5 12 20.5C7.58172 20.5 4 16.9183 4 12.5C4 8.08172 7.58172 4.5 12 4.5C16.4183 4.5 20 8.08172 20 12.5Z" stroke="#c6c6c6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>`;
            dateDiv.appendChild(dateIcon);

            const dateString = document.createElement("p");
            dateString.textContent = this.endDate;
            dateString.className = "text-sm text-slate-500";
        ...
        if (this.priority) {
            ...
            if (this.priority === "critical")
                priorityIcon.className = "fill-red-800";
            else if (this.priority === "high")
                priorityIcon.className = "fill-red-500";
            else if (this.priority === "medium")
                priorityIcon.className = "fill-amber-500";
            else if (this.priority === "low")
                priorityIcon.className = "fill-cyan-500";
            ...
            priorityString.className = "text-sm text-slate-500";
        ...
        if (this.tags.length > 0) {
            ...
            this.tags.forEach((tag) => {
                ...
                tagDisplay.className =
                    "rounded-md bg-bgExtraLight text-textPrimaryLight text-xs px-1 lg:text-sm lg:px-2 lg:py-1";
